...
namespace practice5
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var order = os.CreateOrder("order 1");
            order.AddOrderItem("apple", 10, 9.99);
            os.CreateOrder("order 1");
            os.ExportXML("s.xml");

            OrderService read_os = new OrderService();
            read_os.ImportXML("s.xml");
            foreach(var i in read_os.orderlist)
            {
                Console.WriteLine(i.OrderName);
            }
    ...
    public class OrderService
    {
        ...
        // 导出为XML文件
        public void ExportXML(string file_name)
        {
            XmlSerializer xmlserializer = new XmlSerializer(typeof(List<Order>));
            using (FileStream fw = new FileStream(file_name, FileMode.Create))
            {
                xmlserializer.Serialize(fw, orderlist);
            }
        }
        ...
        {
            XmlSerializer xmlserializer = new XmlSerializer(typeof(List<Order>));
            using (FileStream fw = new FileStream(file_name, FileMode.Open))
            {
                orderlist = (List<Order>)xmlserializer.Deserialize(fw);
            }
        }
    }
